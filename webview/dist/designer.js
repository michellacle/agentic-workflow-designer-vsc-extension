...
(function () {
    ...
    function drawPorts(node, x, y, w, h) {
        ...
        const targetSides = new Set();
        for (const edge of state.workflow.edges) {
            if (edge.source === node.id)
                sourceSides.add(edge.sourceSide || 'right');
            if (edge.target === node.id)
                targetSides.add(edge.targetSide || 'left');
        }
        ...
        if (state.creatingEdge && state.editMode) {
            ...
            if (portHit && portHit.nodeId !== state.creatingEdge.sourceNodeId) {
                ...
                if (sourceNode && targetNode && targetNode.type !== 'start' && sourceNode.type !== 'end') {
                    ...
                    const exists = state.workflow.edges.some(e => e.source === state.creatingEdge.sourceNodeId && e.target === portHit.nodeId);
                    if (!exists) {
    ...
    function updatePropertiesPanel(node) {
        ...
        content.querySelectorAll('input, textarea, select').forEach(input => {
            input.addEventListener('change', (e) => {
                ...
                updateNodeProperty(node, label, target.value);
            });
        });
    }
    ...
    function renderStateMappings(node) {
        const mappings = node.data.stateWrites || [];
        let html = `<div class="property-section">
    ...
    function updateMappingSource(index, value) {
        ...
        if (!node.data.stateWrites)
            return;
        if (!node.data.stateWrites[index])
            return;
    ...
    function getDisplayLabel(node) {
        if (node.type === 'note')
            return node.data.text?.substring(0, 20) || 'Note';
        if (node.type === 'process')
            return node.data.title || 'Process';
        if (node.type === 'decision')
            return node.data.question?.substring(0, 18) || 'Decision';
    ...
    window.addDecisionOption = addDecisionOption;
    window.removeDecisionOption = removeDecisionOption;
    window.updateDecisionOption = updateDecisionOption;
