...
// ===== Validator Tests: HumanApproval edge requirements =====

describe('WorkflowValidator: HumanApproval Node Edge Requirements', () => {
    ...
    it('should pass if HumanApproval node has exactly 2 outgoing edges (Approve/Reject)', () => {
        const workflow: Workflow = {
            ...
            nodes: [
                ...
                { id: 'approved', type: NodeType.Delay, position: { x: 200, y: -50 }, data: { duration: 0 } as DelayNodeData },
                { id: 'rejected', type: NodeType.Delay, position: { x: 200, y: 50 }, data: { duration: 0 } as DelayNodeData },
                { id: 'end', type: NodeType.End, position: { x: 300, y: 0 }, data: {} }
            ],
            edges: [
                { id: 'e1', source: 'start', target: 'approval' },
                { id: 'e2', source: 'approval', target: 'approved', label: 'Approve' },
                { id: 'e3', source: 'approval', target: 'rejected', label: 'Reject' },
                { id: 'e4', source: 'approved', target: 'end' },
                { id: 'e5', source: 'rejected', target: 'end' }
            ]
        };
        const errors = validateWorkflow(workflow);
        const approvalErrors = errors.filter((e: any) =>
            e.node === 'approval' && e.message.includes('HumanApproval')
        );
        expect(approvalErrors.length).toBe(0);
    });

    it('should fail if HumanApproval node has only 1 outgoing edge', () => {
        const workflow: Workflow = {
            name: 'approval-missing-reject',
            nodes: [
                { id: 'start', type: NodeType.Start, position: { x: 0, y: 0 }, data: {} },
                {
                    id: 'approval',
                ...
                { id: 'approved', type: NodeType.Delay, position: { x: 200, y: 0 }, data: { duration: 0 } as DelayNodeData },
                { id: 'end', type: NodeType.End, position: { x: 300, y: 0 }, data: {} }
            ],
            edges: [
                { id: 'e1', source: 'start', target: 'approval' },
                { id: 'e2', source: 'approval', target: 'approved', label: 'Approve' },
                // Missing Reject edge
                { id: 'e3', source: 'approved', target: 'end' }
            ]
        };
        const errors = validateWorkflow(workflow);
        const approvalErrors = errors.filter((e: any) =>
            e.node === 'approval' && e.message.includes('2 outgoing edges')
        );
        expect(approvalErrors.length).toBeGreaterThan(0);
    });

    it('should fail if HumanApproval node has 0 outgoing edges', () => {
        const workflow: Workflow = {
            name: 'approval-no-edges',
            nodes: [
                { id: 'start', type: NodeType.Start, position: { x: 0, y: 0 }, data: {} },
                {
                    id: 'approval',
            ...
            edges: [
                { id: 'e1', source: 'start', target: 'approval' }
                // No outgoing edges from approval
            ]
        };
        const errors = validateWorkflow(workflow);
        const approvalErrors = errors.filter((e: any) =>
    ...
    it('should fail if HumanApproval node has 3+ outgoing edges', () => {
        const workflow: Workflow = {
            name: 'approval-too-many-edges',
            nodes: [
                { id: 'start', type: NodeType.Start, position: { x: 0, y: 0 }, data: {} },
                {
                    id: 'approval',
                ...
                { id: 'a', type: NodeType.Delay, position: { x: 200, y: -100 }, data: { duration: 0 } as DelayNodeData },
                { id: 'b', type: NodeType.Delay, position: { x: 200, y: 0 }, data: { duration: 0 } as DelayNodeData },
                { id: 'c', type: NodeType.Delay, position: { x: 200, y: 100 }, data: { duration: 0 } as DelayNodeData },
                { id: 'end', type: NodeType.End, position: { x: 300, y: 0 }, data: {} }
            ],
            edges: [
                { id: 'e1', source: 'start', target: 'approval' },
                { id: 'e2', source: 'approval', target: 'a', label: 'Approve' },
                { id: 'e3', source: 'approval', target: 'b', label: 'Reject' },
                { id: 'e4', source: 'approval', target: 'c', label: 'Maybe' },
    ...
    it('should not enforce 2-edge rule on non-branching nodes (Agent, Delay)', () => {
        const workflow: Workflow = {
            name: 'agent-one-edge',
            nodes: [
                { id: 'start', type: NodeType.Start, position: { x: 0, y: 0 }, data: {} },
                { id: 'agent', type: NodeType.Agent, position: { x: 100, y: 0 }, data: { agent: 'test' } as AgentNodeData },
        ...
        const errors = validateWorkflow(workflow);
        const branchingErrors = errors.filter((e: any) =>
            e.message.includes('2 outgoing edges')
        );
        expect(branchingErrors.length).toBe(0);
    });
});
...
// ===== Designer: Dual Output Ports for HumanApproval =====

describe('Designer: HumanApproval Dual Output Ports', () => {
    ...
    it('should render dual output ports for human_approval nodes in drawPorts', () => {
        // Check that drawPorts function handles human_approval type
        const hasApprovalPorts = designerContent.includes("node.type === 'human_approval'") ||
                                 designerContent.includes('node.type === "human_approval"');
        expect(hasApprovalPorts).toBe(true);
    });

    it('should have Approve port hit-testing in hitTestOutputPorts for human_approval', () => {
        // Check that hitTestOutputPorts handles human_approval type
        const hasApprovalHitTest = designerContent.includes("node.type === 'human_approval'") ||
                                    designerContent.includes('node.type === "human_approval"');
        expect(hasApprovalHitTest).toBe(true);
    });

    it('should label the Approve port in drawPorts', () => {
        // The Approve label should appear near the human_approval port drawing code
        const drawPortsMatch = designerContent.match(/function drawPorts[\s\S]*?function\s+\w+/);
        expect(drawPortsMatch).not.toBeNull();

        const drawPortsBody = drawPortsMatch![0];
        // Should have 'Approve' text label for the approval port
        expect(drawPortsBody).toMatch(/Approve/i);
    });

    it('should label the Reject port in drawPorts', () => {
        const drawPortsMatch = designerContent.match(/function drawPorts[\s\S]*?function\s+\w+/);
        expect(drawPortsMatch).not.toBeNull();

        const drawPortsBody = drawPortsMatch![0];
        // Should have 'Reject' text label for the approval port
        expect(drawPortsBody).toMatch(/Reject/i);
    });

    it('should return "approve" port from hitTestOutputPorts for human_approval', () => {
        const hitTestMatch = designerContent.match(/function hitTestOutputPorts[\s\S]*?function\s+\w+/);
        expect(hitTestMatch).not.toBeNull();

        const hitTestBody = hitTestMatch![0];
        // Should return port: 'approve' for the approve port
        expect(hitTestBody).toMatch(/port:\s*['"]approve['"]/);
    });

    it('should return "reject" port from hitTestOutputPorts for human_approval', () => {
        const hitTestMatch = designerContent.match(/function hitTestOutputPorts[\s\S]*?function\s+\w+/);
        expect(hitTestMatch).not.toBeNull();

        const hitTestBody = hitTestMatch![0];
        // Should return port: 'reject' for the reject port
        expect(hitTestBody).toMatch(/port:\s*['"]reject['"]/);
    });
});
